...
KEYWORDS = re.compile(r"\b(section|segment|global|globl|extern|db|dw|dd|dq|dt|resb|resw|"
                      r"resd|resq|equ|times|bits|default|align|proc|endp|end|includelib|"
                      r"byte|word|dword|qword|ptr)\b", re.I)
REGISTERS = re.compile(r"\b(r[abcd]x|r[sd]i|r[sb]p|r8|r9|r1[0-5]|e[abcd]x|e[sd]i|e[sb]p|"
                       r"[abcd][lhx]|sil|dil|spl|bpl|r\d+[dwb]|xmm\d+|rip)\b", re.I)
...
class CodeEditor(ttk.Frame):
    ...
    def __init__(self, master, on_change=None, on_breakpoint=None, on_cursor=None):
        ...
        self.gutter = tk.Canvas(self, width=58, bg=theme.PANEL, highlightthickness=0,
                                bd=0, takefocus=0)
        ...
        self.text = tk.Text(self, wrap="none", undo=True, maxundo=-1,
                            bg=theme.BG, fg=theme.FG, insertbackground=theme.ACCENT,
                            selectbackground=theme.SEL, selectforeground=theme.WHITE,
                            font=self.font, borderwidth=0, highlightthickness=0,
                            tabs=("1c",), padx=8, pady=4,
    ...
    def set_code(self, code: str, keep_view=False):
    ...
    def goto_line(self, line: int, focus=True):
    ...
    def _cursor_moved(self, event=None):
        self.mark_current_line()
        if self.on_cursor:
    ...
    def redraw_gutter(self):
        ...
        while True:
            ...
            if line == exec_line:
                self.gutter.create_rectangle(0, y - 1, 58, y + dline[3] + 1,
                                             fill=theme.EXEC_LINE, outline="")
                color = theme.CALL
            if line in self.breakpoints:
                self.gutter.create_oval(6, y + 4, 15, y + 13, fill=theme.BREAK, outline="")
            if str(line) in self.notes:
                self.gutter.create_text(54, y + 8, text="✎", fill=theme.ACCENT,
                                        font=theme.ui(9), anchor="e")
            self.gutter.create_text(40, y + 8, text=str(line), anchor="e",
                                    fill=color, font=theme.mono(9))
            line += 1
    ...
    def mark_error_lines(self, lines):
    ...
    def highlight(self):
        self._highlight_job = None
        t = self.text
        for tag in ("mnemonic", "register", "number", "string", "comment",
                    "label", "directive"):
    ...
    def toggle_comment(self, event=None):
        ...
        comentadas = all(l.strip().startswith(";") or not l.strip() for l in linhas)
        self.text.edit_separator()
    ...
    def insert_at_cursor(self, texto: str):
        self.text.insert("insert", texto)
        self._changed()

    # ------------------------------------------------------------ busca ---
    def find(self, termo: str, from_start=False) -> bool:
