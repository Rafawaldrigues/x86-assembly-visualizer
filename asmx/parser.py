...
from .isa import (CONTROL_DIRECTIVES, DATA_DIRECTIVES, ISA, REG_INFO,
                  SIZE_KEYWORDS)
...
class Program:
    ...
    def instructions(self) -> List[Line]:
        return [l for l in self.lines if l.kind == "instruction"]


LABEL_RE = re.compile(r"^([A-Za-z_.$?@][\w.$@?]*)\s*:\s*(.*)$")
MASM_PROC_RE = re.compile(r"^([A-Za-z_?@][\w?@]*)\s+(proc|endp)\b", re.I)
DATA_DEF_RE = re.compile(
    r"^([A-Za-z_.$][\w.$@]*)\s+(db|dw|dd|dq|dt|resb|resw|resd|resq|equ|times)\b\s*(.*)$",
    re.I)
...
def strip_comment(line: str):
    """Separa código e comentário respeitando strings."""
    ...
    while i < len(line):
        ...
        if ch == ";" or (ch == "#" and not prev.isalnum()) or (ch == "/" and line[i:i + 2] == "//"):
            comment = line[i:]
            break
        out += ch
        i += 1
...
def classify_operand(text: str) -> Operand:
    ...
    if (t.startswith("[") and t.endswith("]")) or re.match(r"^[\w]*\s*ptr\s*\[", t, re.I):
        ...
        op.regs = [r.lower() for r in re.findall(r"\b[A-Za-z][\w]*\b", op.inner)
                   if r.lower() in REG_INFO]
        syms = [s for s in re.findall(r"\b[A-Za-z_.$][\w.$@]*\b", op.inner)
...
def detect_flavor(text: str) -> str:
    ...
    masm = (re.search(r"(^|\n)\s*\.(code|data|model)\b", text, re.I)
            or re.search(r"\bproc\b[\s\S]*\bendp\b", text, re.I))
    if att > 2:
        return "att"
    if masm:
        return "masm"
...
def _att_memory(expr: str) -> str:
    def repl(m):
        ...
        if disp and disp not in ("", "0"):
            out = (out + "+" + disp) if out else disp
        return "[" + out + "]"
    return re.sub(r"(-?\w*)\(([^)]*)\)", repl, expr)


def normalize_att(mnemonic: str, operands: List[str]):
    m = mnemonic.lower()
...
def parse(text: str) -> Program:
    ...
    for i, raw in enumerate(raw_lines):
        ...
        if m and m.group(1).lower() not in SIZE_KEYWORDS:
            ...
            if not entry.local_label:
                func = m.group(1)
            entry.func = func
            symbols.setdefault(m.group(1), {
                "name": m.group(1), "line": i + 1, "type": "label",
                "section": section, "global": False, "local": entry.local_label})
            lines.append(entry)
        ...
        if mp:
            ...
            if entry.directive == "proc":
                func = mp.group(1)
                entry.func = func
                symbols[mp.group(1)] = {"name": mp.group(1), "line": i + 1,
                                        "type": "label", "section": section,
        ...
        if dd:
            ...
            symbols[dd.group(1)] = {"name": dd.group(1), "line": i + 1,
                                    "type": "data", "directive": entry.directive,
                                    "section": section, "global": False,
                                    "local": False}
            lines.append(entry)
            continue
        ...
        if head in CONTROL_DIRECTIVES or head.startswith("%") or \
                (head.startswith(".") and head[1:] not in ISA):
            entry.kind = "directive"
            ...
            if entry.directive in ("global", "globl", "export", "public"):
                for a in entry.args:
                    s = symbols.setdefault(a, {"name": a, "line": i + 1,
                                               "type": "label", "section": section,
                                               "global": False, "local": False})
                    s["global"] = True
            if entry.directive.startswith("extern"):
                for a in entry.args:
                    symbols[a] = {"name": a, "line": i + 1, "type": "extern",
                                  "section": section, "global": True, "local": False}
            lines.append(entry)
            continue
    ...
    return Program(lines=lines, symbols=symbols, flavor=flavor, source=text)
